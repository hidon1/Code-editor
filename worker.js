 ...
 *   XAI_API_KEY    — מפתח xAI שלך (מומלץ בצד שרת)
 *   ALLOWED_ORIGIN — אופציונלי, לדוגמה: https://code-editor-4c979.web.app
 *   XAI_MODEL      — אופציונלי, ברירת מחדל: grok-3-mini-fast
 *
 * Optional request header from client:
 *   x-xai-api-key  — מפתח xAI מהלקוח (fallback אם אין env.XAI_API_KEY)
 */
...
async function callXAI(env, request, messages) {
  const apiKey = env.XAI_API_KEY || request.headers.get('x-xai-api-key') || '';
  if (!apiKey) {
    throw new Error('חסר XAI_API_KEY בסביבת ה-Worker או בכותרת x-xai-api-key');
  }
...
async function handleImprove(request, env) {
  ...
  const filesBlock = body.files.map(f => `=== ${f.name} ===\n${f.content || ''}`).join('\n\n');
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      return withCors(apiError(e?.message || 'שגיאת שרת', 500), cors);
    }
  }
};
